...
REGISTRY: list[Integration] = [
    ...
    Integration("pinterest", "Pinterest", "Marketing",
                [Field("app_id", "App ID", secret=True, env="PINTEREST_APP_ID"),
                 Field("app_secret", "App Secret Key", secret=True, env="PINTEREST_APP_SECRET"),
                 Field("redirect_uri", "Redirect URI", env="PINTEREST_REDIRECT_URI"),
                 Field("board_id", "Board ID", env="PINTEREST_BOARD_ID"),
                 Field("access_token", "Access Token (set by Connect)", secret=True,
                       env="PINTEREST_ACCESS_TOKEN")],
                required=["access_token"], actions=["test", "reconnect_oauth"],
                setup=["Create a Pinterest app (production, not sandbox)",
                       "Paste its App ID + App Secret + a Redirect URI registered on the app",
                       "Add your target Board ID",
                       "Click 'Reconnect OAuth' → approve pins:write in the browser",
                       "Test Connection"]),
    Integration("facebook", "Facebook", "Marketing",
                [Field("page_access_token", "Page Access Token", secret=True,
                 ...
                 Field("app_id", "App ID (for token refresh)", env="META_APP_ID"),
                 Field("app_secret", "App Secret (for token refresh)", secret=True,
                       env="META_APP_SECRET")],
                ...
                setup=["Create a Meta app", "Get a Page access token",
                       ...
                       "For a token that never expires: add App ID + Secret, then "
                       "run `python main.py --facebook-token <short-lived-token>`"]),
