...
REGISTRY: list[Integration] = [
    ...
    Integration("shopify", "Shopify", "Commerce",
                [Field("store_domain", "Store URL", env="SHOPIFY_STORE_DOMAIN"),
                 Field("admin_token", "Admin Access Token", secret=True, env="SHOPIFY_ADMIN_TOKEN"),
                 Field("blog_id", "Blog ID", env="")],
                required=["store_domain", "admin_token"],
                actions=["test", "publish_test_product"],
                setup=["Shopify admin → Apps → Develop apps → Create a custom app",
                       "Grant Products + Content scopes; install the app",
                       "Copy the Admin API access token",
                       "Paste store URL + token here", "Test Connection"]),
